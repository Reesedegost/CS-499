...
class TreasureMaze(object):
    ...
    def get_reward(self):
        ...
        # Positive reinforcement: reaching the goal
        if (pirate_row, pirate_col) == self.target:
            return 10.0

        # New: add distance-based incentive
        ...
        if (pirate_row, pirate_col) in self.visited:
            return -0.5  # more punishing
        if mode == 'invalid':
            return -1.0  # stronger penalty
        if mode == 'valid':
            return distance_reward  # variable incentive
